...
Acceptance checklist for the Account cycle against `https://stg.api.brooch.sa`.
...
| login-single-multi-app | Login (one tenant, multiple apps) | Stay in Account → applications launcher |
| login-multi-tenant | Login (multiple selectable companies) | `/select-company` (wins over `redirectUrl`) |
| login-intent | Sibling app → Account `?intentId=` → login | Navigable `redirectUrl` to target app |
...
| intent-expired | Expired `intentId` on login | `Account.LoginIntent.Expired` → scrub URL |
| intent-replayed | Already-completed intent | `Account.LoginIntent.AlreadyCompleted` |

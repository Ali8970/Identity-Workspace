  ...
  private async load(): Promise<void> {
    ...
    try {
      const [members, roles] = await Promise.all([
        firstValueFrom(this.membersApi.list(tenantId)),
        firstValueFrom(this.rolesApi.list(tenantId)),
      ]);
  ...
  protected onAdd(event: Event): void {
    ...
    void submit(this.addForm, async () => {
      ...
      try {
        ...
        const identityRole = this.roles().find((r) => r.applicationKey === 'identity');
        const result = await firstValueFrom(
          this.membersApi.add(tenantId, {
            email: value.email.trim(),
            arabicName: value.arabicName.trim(),
            englishName: value.englishName.trim(),
            roleIds: identityRole ? [identityRole.id] : [],
          }),
        );

...
import { PackageDto, TenantApi } from '../../core/api/workspace-api.service';
...
export class OnboardingPage {
  ...
  private async bootstrap(): Promise<void> {
    try {
      const tenant = await firstValueFrom(this.tenantApi.get());
      ...
      if (this.step() === 'package') {
        const packages = await firstValueFrom(this.tenantApi.packages());
  ...
  protected async startTrial(): Promise<void> {
    ...
    this.error.set(null);
    try {
      await firstValueFrom(this.tenantApi.startFreeTrial(packageId));
